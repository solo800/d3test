...
(function (SmgLine) {
    var Graph = (function () {
        ...
        Graph.prototype.setAxisData = function (data) {
        ...
        Graph.prototype.setConfig = function (config) {
            if ('undefined' === typeof config) {
                ...
                if ('undefined' === typeof config.formatData) {
                    this.config.formatData = function (data) {
                        return data;
                    };
                }
                else {
        ...
        Graph.prototype.drawGraph = function () {
            var svg = d3.select(this.config.graphContainer).append('svg')
                ...
                .append('g').attr('transform', 'translate(' + this.config.margin.left + ',' + this.config.margin.top + ')'), dim = {
                margin: this.config.margin,
                padding: this.config.padding,
                height: this.config.height,
                width: this.config.width
            }, lineColor, dotColor, colorIndex, 
            //Add tooltip div
        ...
        Graph.prototype.addDots = function (data, svg, dim, color, dotRadius, tooltip) {
            var yMax;
            this.data.forEach(function (ds) {
                ds.forEach(function (d) {
                    if ('undefined' === typeof yMax)
                        yMax = d.value;
                    if (d.value > yMax)
                        yMax = d.value;
                });
            });
            var width = dim.width, height = dim.height, xScale = d3.scale.linear()
                .domain([0, data.length])
                .range([0, width + dim.margin.right])
                .nice(), yScale = d3.scale.linear()
                .domain([0, yMax])
                .range([height, 0])
                .nice();
        ...
        Graph.prototype.buildLine = function (data, svg, dim, color, strokeWidth) {
            var yMax;
            this.data.forEach(function (ds) {
                ds.forEach(function (d) {
                    if ('undefined' === typeof yMax)
            ...
            var margin = dim.margin, width = dim.width, height = dim.height, xScale = d3.scale.linear()
                .domain([0, data.length])
                .range([0, width + margin.right])
                .nice(), yScale = d3.scale.linear()
                .domain([0, yMax])
                .range([height, 0])
                .nice(), lineFn = d3.svg.line()
        ...
        Graph.prototype.axisGen = function (xAxisData, svg, dim, axisConfig) {
            var yMax = 0;
            this.data.forEach(function (ds) {
                ds.forEach(function (d) {
                    if (d.value > yMax)
            ...
            var margin = dim.margin, height = dim.height, width = dim.width, xScale = d3.scale.ordinal()
                .domain(xAxisData)
                .rangePoints([0, width + margin.right]), yScale = d3.scale.linear()
                .domain([0, yMax])
                .range([height, 0]), xGen = d3.svg.axis().scale(xScale).orient(axisConfig.x.orientation), yGen = d3.svg.axis().scale(yScale).orient(axisConfig.y.orientation), xAxis = svg.append('g').call(xGen)
                .attr({
        ...
        Graph.prototype.alignXAxis = function () {
            var ticks = $('.axis.line-x > g > text'), dotSet = {}, dots = $(this.config.graphContainer + ' > svg > g').children('circle'), i = 0, diff;
        ...
        Graph.prototype.formatAxisData = function (data) {
        ...
        Graph.prototype.normalizeData = function () {
            var data = this.data, maxDataSet = 0, diff;
            data.forEach(function (dataSet) {
                if (dataSet.length > maxDataSet)
