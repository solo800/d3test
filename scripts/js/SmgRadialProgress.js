...
var SmgRadialProgress;
(function (SmgRadialProgress) {
    var Graph = (function () {
        function Graph(data, config) {
            ...
            this.graphConId = '#graph-area-radial';
        ...
        Graph.prototype.setConfig = function (config) {
            if ('undefined' === typeof config) {
                ...
                if (this.config.graphContainer.slice(0, 1).indexOf('#') === -1)
                    this.config.graphContainer = '#' + this.config.graphContainer;
        ...
        Graph.prototype.radialProgress = function (parent) {
            var config = this.config, _data = null, _duration = config.duration, _selection, _margin = config.margin, __width = config.width, __height = config.height, _diameter = config.diameter, _kpiName = this.data.name, _fontSize = config.fontSize, _value = 0, _minValue = 0, _maxValue = 100;
                ...
                _selection.each(function (data) {
                    ...
                    for (var m in config.margin) {
                        if (config.margin.hasOwnProperty(m)) {
                            if (0 !== config.margin[m])
                                addMargin = true;
                        }
                    }
                    if (addMargin)
                        enter.style('margin', config.margin.top + ' ' + config.margin.right + ' ' + config.margin.bottom + ' ' + config.margin.left);
                    enter.append("g");
                    measure();
                    ...
                    path.enter().append("path")
                        ...
                        .attr("transform", "translate(" + _width / 2 + "," + _width / 2 + ")")
                        .attr("d", _arc)
                        .attr('fill', function () { return setColor(config.colors, _value[0]); });
            ...
            function setColor(colors, value) {
                for (var c in colors) {
    ...
    SmgRadialProgress.Graph = Graph;
})(SmgRadialProgress || (SmgRadialProgress = {}));
