...
module SmgStackedBar {
	interface IStackGraph {
		data?: any[];
		scaleData?: any[];
		config?: IStackConfig;
		...
		defConMargin: {};
		defaultConfig: IStackConfig;
		setData: (data: any) => void;
		toArray: (obj: any) => any[];
		toObj: (values: any, obj: {}) => {};
		defaultFormatScaleData: (data: number[]) => any[];
	}
	interface IStackConfig {
		formatData?: (data: any) => number[];
		formatScaleData?: (data: number[]) => any[];
		colors?: string[];
		graphContainer?: string; //Expects DOM element ID
		margin?: IStackMargin;
		padding?: number;
		height?: number;
		width?: number;
	...
	interface IStackMargin {
		top: number,
		right: number,
		bottom: number,
		left: number;
	}
	export class Graph implements IStackGraph {
		constructor(public data: any[], public config: IStackConfig) {
			this.setConfig(config);
			this.setData(data);
		}

		setConfig(config: IStackConfig) {
		...
		defConGraphContainerId = '#graph-area-stacked-bar'
		...
		setData(data: any) {
			if ('[object Array]' !== typeof Object.prototype.toString.call(data)) data = this.toArray(data);
		...
		drawGraph() {
			var xScale = d3.scale.linear()
				...
				xAxisScale = d3.scale.ordinal()
					...
					.rangePoints([0, this.config.width + this.config.margin.right]),
		...
		defaultFormatScaleData(data: number[]) {
			var scaleData = [],
				numCols = this.config.axis.x.values.length,
				numDataPoints = this.data.length / numCols,
				c = 0,
				dp;
